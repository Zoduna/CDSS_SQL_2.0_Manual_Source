...
result = _db.exec("SELECT TIME('23:23:23', 'start of day') from
dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
...
_rs.resultsetrelease()
result = _db.exec("SELECT TIME('23:23:23', '1 seconds', '2 minutes', '1
hours') from dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
...
_rs.resultsetrelease()
result = _db.exec("SELECT TIME('23:23:23', '-1 seconds', '-2 minutes',
'-1 hours') from dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
