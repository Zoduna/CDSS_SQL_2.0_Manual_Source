...
_db = new ZodunaDBSQL_Databasedatabase("testfolder/"); 
bool isOpen = _db.open("master", @"master"); //returns "true" if successful and "false" if the command fails. 
if (!isOpen) 
{ 
	printf("zodunadb error while opening DataStore\n"); 
	return; 
}
bool isdone = _db.modifypassword(
	"master"
	, "master" 
	,"newmaster"
	,0 
