...
_db.commit(); 
result = _db.exec("SELECT current_date() FROM dumtab"); 
...
_rs.release();
result = _db.exec("SELECT (3+4)*5 FROM dumtab"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
...
_rs.release();
result = _db.exec("SELECT current_date(), current_time() FROM dumtab"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
...
_rs.release();
result = _db.exec("SELECT abs(-5) FROM dumtab"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
