...
_db.commit();
result = _db.exec("SELECT JULIANDAY('2010-12-31 01:10:00', 'start of day') from dumtab");
_rs = _db.getresultset();
while (_rs.getnext() ==ZodunaDBSQL_Database.ZODUNADB_ROW) 
{ 
	double d1= _rs.getdouble(1);
	System.out.println("zodunadb test col1 = %f", 
		d1);
}
_rs.resultsetrelease();
result = _db.exec("SELECT JULIANDAY('2010-12-31 01:00:00', 'start of month') from dumtab");
_rs = _db.getresultset();
while (_rs.getnext() ==ZodunaDBSQL_Database.ZODUNADB_ROW) 
{ 
	double d1= _rs.getdouble(1);
...
_rs.resultsetrelease();
result = _db.exec("SELECT JULIANDAY('-4713-11-24') from dumtab");
_rs = _db.getresultset();
while (_rs.getnext() ==ZodunaDBSQL_Database.ZODUNADB_ROW) 
{ 
	double d1= _rs.getdouble(1);
...
_rs.resultsetrelease();
result = _db.exec("SELECT JULIANDAY('-4712-11-24') from dumtab");
_rs = _db.getresultset();
while (_rs.getnext() ==ZodunaDBSQL_Database.ZODUNADB_ROW) 
{ 
	double d1= _rs.getdouble(1);
