...
where the DataStore is to be accessed
...
//use createadminuser only when creating
the DataStore

rc = _db.createadminuser("master",
password:"master", encryption_algorithm: ZODUNADB_ENCRYPTION_TEA,  encryptpassword:"key for encryption")

 

//used every time to create/open encrypted
DataStore
let isOpen = _db.open_encrypted("master",
password:"master",encryption_algorithm: ZODUNADB_ENCRYPTION_TEA ,encryptpassword:"key
for encryption") //returns
