...
result = _db.exec("CREATE SCHEMA
db2.test") 

result = _db.exec("create
table test.table2 (col1 int)")

_db.begintransaction()

result = _db.exec("insert
into table1 values (5)")

result = _db.exec("insert
into table1 values (7)")

result = _db.exec("insert
into table1 values (9)")

result = _db.exec("insert
into table1 values (11)")

result = _db.exec("insert
into table1 values (15)")

result = _db.exec("insert
into test.table2 values (3)")

result = _db.exec("insert
into test.table2 values (5)")

result = _db.exec("insert
into test.table2 values (11)")

result = _db.exec("insert
into test.table2 values (17)")

result = _db.exec("insert
into test.table2 values (19)")

_db.commit()
result = _db.exec("SELECT a.col1, b.col1 FROM table1 a NATURAL LEFT
OUTER JOIN test.table2 b")
