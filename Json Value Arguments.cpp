...
result = _db->exec("SELECT json_object('ex','[52,3.14159]') FROM dumtab"); 
...
result = _db->exec("SELECT json_object('ex',json('[52,3.14159]')) FROM dumtab"); 
_rs = _db->getresultset();
while (_rs.getnext() == ZodunaDBSQL_Constants::ZODUNADB_ROW) 
{ 
	char*    text1 = _rs gettext(1); 
...
_rs->resultsetrelease();
result = _db->exec("SELECT json_object('ex',json_array(52,3.14159)) FROM dumtab"); 
_rs = _db->getresultset();
while (_rs.getnext() == ZodunaDBSQL_Constants::ZODUNADB_ROW) 
{ 
	char*    text1 = _rs gettext(1); 
