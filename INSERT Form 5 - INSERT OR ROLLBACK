...
boolean result = _db.exec("CREATE TABLE Customers(Customer VARCHAR(100), City VARCHAR(100), Country VARCHAR(100))");
result = _db.exec("CREATE TABLE table1 (col1 int, col2 VARCHAR(100))");
result = _db.exec("CREATE UNIQUE INDEX idx1 ON table1 (col1)");
result = _db.exec("CREATE UNIQUE INDEX idx2 ON Customers (Customer)");
_db.begintransaction();
result = _db.exec("INSERT INTO table1 VALUES(1234, 'abcd')");
result = _db.exec("INSERT INTO table1 VALUES(2345, 'efgh')");
result = _db.exec("INSERT INTO Customers (CustomerName, City, Country) VALUES ('Cardinal', 'Stavanger', 'Norway')");
result = _db.exec("INSERT OR ROLLBACK INTO Customers (City, Country, CustomerName) VALUES ('Stavanger', 'Norway', 'Cardinal')");
result = _db.exec("INSERT OR ROLLBACK INTO Customers (City, Country, CustomerName) VALUES ('Stavanger', 'Norway', 'Cardinal')");
_db.commit();
