...
_db.commit()
result = _db.exec("SELECT UPPER('ABC') AS 'col1', upper('abc') AS
'col2', upper('123') AS 'col3' from dumtab ") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
                let  text1 = _rs.gettext(1) 

                let text2 = _rs.gettext(2) 

                let text3 = _rs.gettext(3) 

                println("zodunadb
test col1 = \(text1), col2 = \(text2), col3 = \(text3)") 
}
