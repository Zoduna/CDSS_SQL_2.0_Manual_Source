...
IZodunaDBSQL_ResultSet _rs; 
IZodunaDBSQL_Statement _stmt; 
int		rc = 0;
...
rc = _db.createadminuser(this.getFilesDir().getPath() + this.getPackageName() + "/", "master", "master", ZodunaDBSQL_Database.ZODUNADB_ENCRYPTION_AES_128 ,"key for encryption");

//used every time to create/open encrypted DataStore
boolean isOpen = _db.open_encrypted("master" ,"master", ZodunaDBSQL_Database.ZODUNADB_ENCRYPTION_AES_128, "key for encryption"); //returns "true" if successful and "false" if the command fails.
if (!isOpen) 
{ 
