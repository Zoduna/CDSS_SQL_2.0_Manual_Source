...
_db.commit()
result = _db.exec("SELECT json_object('a',2,'c',4) FROM dumtab") 
...
result = _db.exec("SELECT json_object('a',2,'c','{e:5}') FROM dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
	let  text1 = _rs.gettext(1) 
...
_rs.resultsetrelease()
result = _db.exec("SELECT json_object('a',2,'c',json_object('e',5)) FROM dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
	let  text1 = _rs.gettext(1) 
