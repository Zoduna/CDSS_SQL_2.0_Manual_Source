...
table2 (col1 byte, col2 short, col3 int, col4 long, col5 float, col6
double)") 
...
result = _db.exec("CREATE
table table10 (col1 int not null, primary key (col1), constraint key_check
check (col1 > 10)") 

result = _db.exec("CREATE
table table20 (col2 int, constraint key_reference foreign key (col2) references
table10 (col1) on delete cascade on update cascade)") 

result = _db.exec("CREATE
table table30 (col2 int, constraint key_reference foreign key (col2) references
table10 (col1) on delete SET NULL on update cascade)") 

result = _db.exec("CREATE
table table40 (col2 int, constraint key_reference foreign key (col2) references
table10 (col1) on delete SET DEFAULT on update cascade)") 

result = _db.exec("CREATE
table table20 (col2 int, constraint key_reference foreign key (col2) references
table10 (col1) on delete  NO ACTION on
update cascade)") 
