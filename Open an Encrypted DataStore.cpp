...
rc = _db->createadminuser("master", "master", ZodunaDB_Constants::ZODUNADB_ENCRYPTION_AES_128 ,"key for encryption" passwordretrievalphrase:@"used for retrieval of password");
//used every time to create/open encrypted DataStore
bool isOpen = _db->open_encrypted("master","master", ZodunaDB_Constants::ZODUNADB_ENCRYPTION_AES_128,"key for encryption"); //returns "true" if successful and "false" if the command fails.
if (!isOpen) 
{ 
