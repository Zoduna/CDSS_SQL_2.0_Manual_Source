...
_db.createadminuser(this.getFilesDir().getPath() + this.getPackageName() + "/", "master", "master", 
	ZodunaDBSQL_Database.ZODUNADB_ENCRYPTION_TEA,
	"key for encryption");

boolean isOpen = _db.open_encrypted("master", "master", 
	ZodunaDBSQL_Database.ZODUNADB_ENCRYPTION_TEA,
...
boolean iscreated = _db.createuser("master", "master",  "key for encryption", "user1",
		"password1", ZodunaDBSQL_Database.ZODUNADB_ENCRYPTION_TEA,
		"key for user encryption");
if (!iscreated) 
{ 
	System.out.println("zodunadb error while creating user\n"); 
	return;
}
