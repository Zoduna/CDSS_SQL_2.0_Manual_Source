...
result = _db.exec("CREATE TABLE test.table1 (col1
int, col2 VARCHAR(100))") 

result = _db.exec("CREATE TABLE test.table2(col1
int, col2 VARCHAR(100))") 

result = _db.exec("CREATE UNIQUE INDEX test.idx1
ON table1 (col1)")

result = _db.exec("CREATE
TRIGGER test.trig1 BEFORE INSERT ON table1 BEGIN INSERT INTO table2
VALUES(new.col1, new.col2); END")
_db.begintransaction()
result = _db.exec("INSERT INTO test.table1 VALUES(1234,
'abcd')") 
result = _db.exec("INSERT INTO test.table1 VALUES(2345,
'efgh')") 
_db.commit()
result = _db.exec("SELECT col1, col2 FROM test.table1 WHERE col1 =
1234") 
