...
_db.commit(); 
result = _db.exec("SELECT typeof(12) AS 'col1'  from dumtab");
...
_rs.release();
result = _db.exec("SELECT typeof(NULL) AS 'col1'  from dumtab"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	string    text1 = _rs.gettext(1); 
...
_rs.release();
result = _db.exec("SELECT typeof('abc') AS 'col1'  from dumtab"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	string    text1 = _rs.gettext(1); 
