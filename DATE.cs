...
_db.commit(); 
result = _db.exec("SELECT DATE('2010-12-31 01:00:00', '86400 seconds', '1440 minutes', '24 hours') from dumtab"); 
...
_rs.release();
result = _db.exec("SELECT DATE('2010-12-31 01:00:00', 'start of month') from dumtab "); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	string    text1 = _rs.gettext(1); 
...
_rs.release();
result = _db.exec("SELECT DATE('now', '+86400 seconds', '+1440 minutes', '+24 hours') from dumtab ");
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	string    text1 = _rs.gettext(1); 
