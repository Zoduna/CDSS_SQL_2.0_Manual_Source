...
bool result = _db.exec("CREATE TABLE table1 (col1 varchar(10), col2 varchar(10))"); 
_db.begintransaction(); 
result = _db.exec("INSERT INTO table1 (col2) VALUES ('abcd')"); 
_db.commit(); 
result = _db.exec("SELECT ISNULL(col1) as 'col1' from table1"); 
...
_rs.release();
result = _db.exec("SELECT ISNULL(col2) as 'col1' from table1"); 
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
