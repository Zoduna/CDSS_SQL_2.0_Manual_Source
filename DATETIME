...
_db.commit()
result = _db.exec("SELECT DATETIME('2010-12-31 01:00:00', '86400 seconds',
'1440 minutes', '24 hours') from dumtab") 
...
result = _db.exec("SELECT DATETIME('2010-12-31 01:00:00', '-86400
seconds', '-1440 minutes', '-24 hours') FROM dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
                let  text1 = _rs.gettext(1) 
...
result = _db.exec("SELECT DATETIME('2010-12-31 01:00:00', 'start of
month') FROM dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
...
_rs.resultsetrelease()
result = _db.exec("SELECT DATETIME('now', '+86400 seconds', '+1440
minutes', '+24 hours') from dumtab") 
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
                let  text1 = _rs.gettext(1) 
