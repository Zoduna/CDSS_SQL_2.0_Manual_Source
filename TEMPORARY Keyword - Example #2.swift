...
result = _db.exec("CREATE TEMP TABLE table2 (col1
int, col2 VARCHAR(100))")

result = _db.exec("CREATE TEMPORARY TABLE table3
(col1 int, col2 VARCHAR(100))")

result = _db.exec("CREATE TEMP TABLE IF NOT
EXISTS table4 (col1 int, col2 VARCHAR(100))")
...
TEMPORARY TABLE table5 AS SELECT col1, col2 FROM table1") 
_db.close()
