...
result = _db.exec("CREATE table test.table2
(col1 int)")

result = _db.exec("CREATE INDEX idx1 ON
table1(col1)")

result = _db.exec("CREATE INDEX idx2 ON
test.table2(col2)")
...
result = _db.exec("INSERT into test.table2 values
(3)")
...
_db.commit()
result = _db.exec("SELECT a.col1, b.col1 FROM table1 AS a NATURAL JOIN
test.table2 b")
