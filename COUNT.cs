...
_db.commit(); 
result = _db.exec("SELECT COUNT(col1) AS 'col1' from table1");
...
_rs.release();
result = _db.exec("SELECT COUNT(DISTINCT col1) AS 'col1' from table1");
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
...
_rs.release();
result = _db.exec("SELECT COUNT(*) AS 'col1' from table1");
_rs = _db.getresultset(); 
while (_rs.getnext() == ZodunaDBOO_Database.ZODUNADB_ROW) 
{ 
	int	int1 = _rs.getint(1); 
