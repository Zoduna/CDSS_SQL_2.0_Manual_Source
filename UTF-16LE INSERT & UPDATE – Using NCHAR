...
result = _db.exec("CREATE TABLE table1 (col1 int, col2 NCHAR)")
_db.begintransaction()
_stmt = _db.prepare("INSERT INTO table1 VALUES (?, ?)")
_stmt.bind_int(1, value:25)
_stmt.bind_text16:2 value:s, n:s.length(), enc:ZODUNADB_UTF16LE)
_stmt.step()
_db.commit()
...
_stmt.bind_text16(1, value:s, n:s.length(), enc:ZODUNADB_UTF16LE)
_stmt.step()
_db.commit()
