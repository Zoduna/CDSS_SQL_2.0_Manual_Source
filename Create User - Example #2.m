...
BOOL isOpen = [_db open:@"master" password:@"master"]; //returns "true" if successful and "false" if the command fails. 
if (!isOpen) 
{ 
	printf("zodunadb error while opening DataStore\n"); 
	return; 
}
BOOL iscreated = [_db createuser: @"master" adminpassword:@"master" 
			adminencryptpassword:NULL
			user:@"user1" password:@"password1" 
			encryption_algorithm:0
			userencryptpassword:NULL];
if (!iscreated)
{
	printf("zodunadb error while creating user\n"); 
	return; 
}
