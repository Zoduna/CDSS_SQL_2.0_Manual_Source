...
ADDMONTHS(col1, 5) AS 'col1' from table1")
...
result = _db.exec("SELECT ADDMONTHS(col1, -5) AS 'col1' from
table1")
_rs = _db.getresultset()
while (_rs.getnext() == ZODUNADB_ROW) 
{ 
...
result = _db.exec("DELETE FROM
table1") 

result = _db.exec("INSERT INTO
table1 VALUES('2012-02-27')")
